...
def save_label_for_text():
    ...
    #prediction = prediction+content["Prediction"]
    #effort = effort+content["Effort"]
    cursors = db.cursor()
    #insert_sql="INSERT INTO `annotationdbname`.`textdata` (`id`,`annotation`, `human_effort`, `prediction`, `model_effort`) VALUES (%s,%s,%s,%s,%s);"
    insert_sql = "UPDATE `try`.`textdata` SET annotation=%s, human_effort=%s, prediction=%s, model_effort=%s WHERE id=%s;"
    cursors.execute(insert_sql, (choice, timer, prediction, effort, old_id))
...
def new_function(id):
    ...
    if(int(id)>countofvalue[0]):
        return jsonify({'id': 0, 'data':"no data", 'annotation':"no data", 'human_effort':"no data",'prediction':"no data",'model_effort':"no data"})
    ...
    model_effort = results[5]
    return jsonify({'id': id, 'data':data, 'annotation':annotation, 'human_effort':human_effort,'prediction':prediction,'model_effort':model_effort})
